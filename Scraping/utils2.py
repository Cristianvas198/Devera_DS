...
def scrapear_web(web):
    ...
    try:
        ...
        try:
            # Intenta encontrar un botón típico de consentimiento
            botones = driver.find_elements(By.XPATH, "//button[contains(translate(., 'ACEPTARaccept','aceptaraccept'), 'aceptar') or contains(translate(., 'ACEPTARaccept','aceptaraccept'), 'accept')]")
            for boton in botones:
                try:
                    boton.click()
                    print("Cookies aceptadas.")
                    time.sleep(2)  # espera tras hacer clic
                    break
...
'''def extraer_productos(html_cont):
    """Extrae todos los enlaces con texto y href, sin filtrar."""
    sopa = BeautifulSoup(html_cont, "html.parser")
    productos = []
    for a in sopa.find_all('a', href=True):
        ...
        href = a['href']
        if nombre and href:
            productos.append({"name": nombre, "href": href})
    return productos'''
...
'''def parsear(chunks, description):
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        parsed_results.append(respuesta.content.strip())
